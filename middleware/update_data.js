...
const hourly_Update_Stock_Price = async ()=>{
    ...
    if(isDuringMarketHours){
        const stock_names = await client.query(`SELECT name FROM stock`);
        if(stock_names.rows){
            stock_names.rows.forEach( async stock=>{
                ...
                if(dataFromYahoo){
                    ...
                    client.query(`UPDATE stock SET price = $1 WHERE name=$2`,[price,stock.name],(err)=>{if(!err)console.log(`Updated ${stock.name} price:${price}`)});
                }
            })
            //log 
            client.query(`UPDATE logs SET  hourly_stock_price_updated = $1`,[date.toLocaleString('en-US', {
...
const  update_yahoo_dividend_dates = async ()=>{
    ...
    const client = await db.connect()
    const stock_names = await client.query(`SELECT name FROM stock WHERE is_date_in_yahoo = true`);
        if(stock_names.rows){
            stock_names.rows.forEach( async stock =>{
                ...
                if(dataFromYahoo && !isMonthlyPayer){
                    ...
                    client.query(`UPDATE stock SET yield=$1,dividenddate=$2,monthly_payer=$4, dividend_amount=$5 WHERE name=$3`,[yields,dividendDate,stock.name,isMonthlyPayer,reinvestment_amount],
                            (err)=>{if(!err)console.log(`updated ${stock.name} dividend data: ${yields}`)});
                            
                    //log data
                    const date = new Date();
                    client.query(`UPDATE logs SET  monthly_update_dividend_data_updated = $1`,[date.toLocaleString('en-US', {
                        timeZone: 'America/New_York',
                    })],(err)=>{
...
const update_total_dividends_earned = async ()=>{
    ...
    if(update_quantity_list.rows.length > 0) {
        ...
        client.query(`UPDATE logs SET  yahoo_dividends_earned_updated = $1`,[[date.toLocaleString('en-US', {
            timeZone: 'America/New_York',
        }),...stock_names]],(err)=>{
            (err)?console.log(err.message):console.log('success update stock price')});
        }
...
const update_user_entered_total_dividends_earned = async ()=>{
    ...
    const client = await db.connect()
    let update_quantity_list = await client.query(`SELECT * FROM user_stocks u INNER JOIN stock s ON u.name = s.name WHERE u.user_dividend_date = $1`,[today]);
    update_quantity_list.rows.forEach(async (stock)=>{
    ...
    if(update_quantity_list.rows.length > 0) {
        ...
        client.query(`UPDATE logs SET  user_entered_dividends_earned_updated = $1`,[[date.toLocaleString('en-US', {
            timeZone: 'America/New_York',
        }),...stock_names]]);
        }
    // release the client
...
const update_user_entered_dividend_dates = async ()=>{
    let today = new Date().toISOString().slice(0,10);
    const client = await db.connect()
    const stock_list = await client.query(`select * from user_stocks where user_dividend_date != '' AND user_dividend_date <= $1`,[today]);
    ...
    stock_list.rows.forEach(async stock=>{
        if(stock.total > 0 && stock.user_dividend_date.substring(5,7) !== today.substring(5,7)){// stock pays dividends
            ...
            client.query(`UPDATE user_stocks SET user_dividend_date = $1 WHERE user_id = $2 AND name = $3`,[nextDate,stock.user_id,stock.name],(err)=>{if(err)console.log(err)})
        }
    })
    ...
    if(stock_list.rows.length > 0) {
        ...
        client.query(`UPDATE logs SET  user_entered_dividend_date_updated = $1`,[[date.toLocaleString('en-US', {
            timeZone: 'America/New_York',
        }),...stock_names]]);
        }
    // release the client
...
const setMonthlyDividendRecords = async ()=>{
    ...
    users.rows.forEach(async user =>{
        ...
        const stock_list =  await client.query('SELECT  * FROM user_stocks u INNER JOIN stock s ON u.name = s.name WHERE u.user_id = $1',[user.user_id]);
...
const updateDividendDateAndDateList = async(ticker,nextDate,payment_dates=[])=>{
    if(!nextDate) return 
    const client = await db.connect()
    await client.query(`UPDATE stock SET dividenddate = $2 , date_list = $3 WHERE name = $1`,[ticker.toUpperCase(),nextDate,payment_dates]).catch(e=>console.log('error updating stock date',e.err))
    // release the client
    client.release()
}
...
const updateDividendDatesNotInYahoo = async ()=>{
    try{
        ...
        const client = await db.connect()
        let stock_list = await client.query(`SELECT name,dividenddate,date_list FROM stock WHERE is_date_in_yahoo = false and yield > 0`)
        // release the client
        client.release()
...
const insertNewStockInDB = async (req,res,stock_name)=>{
    ...
    try{ // Inserting new Stock data to stock db
        ...
        client.query(insert,data_array,async (err,result)=>{
            if(result){
...
const addStockToUserStockList = async(uid, stock_name, quantity)=>{
    const client = await db.connect()
    const yields = await client.query(`SELECT yield FROM stock WHERE name = $1`,[stock_name.toUpperCase()]);
    let   estimated_Annual_dividends = parseFloat(yields.rows[0].yield*quantity).toFixed(4) || 0;
    const getDataFromUserTable = await client.query(`SELECT * FROM user_stocks WHERE name = $1 AND user_id = $2`,[stock_name,uid]);
    const isStockAlreadyInUserTable = getDataFromUserTable.rows.length !== 0
    try{
        if(isStockAlreadyInUserTable){
            // update the stock quantity 
            client.query(`UPDATE user_stocks SET quantity = $1 , total = $4 WHERE user_id = $2 AND name = $3`,
            ...
            // insert the stock 
            client.query(`INSERT INTO user_stocks (user_id,name,quantity,total) VALUES ($1,$2,$3,$4)`,
